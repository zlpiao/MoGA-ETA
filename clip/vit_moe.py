...
def createConvFunc(op_type):
    assert op_type in ['cv', 'cd', 'hvd', 'dcd', 'ad'], 'unknown op type: %s' % str(op_type)
    if op_type == 'cv':
        return F.conv2d
    ...
    if op_type == 'cd':
        ...
        def func(x, weights, bias=None, stride=1, padding=0, dilation=1, groups=1):
            theta = 0.7
            #[C_out,C_in,H_k,W_k] = self.conv.weight.shape
            [C_out,C_in,H_k,W_k] = weights.shape
            #import pdb;pdb.set_trace()
            tensor_zeros = torch.FloatTensor(C_out, C_in, 1).fill_(0).cuda()
            conv_weight = torch.cat((tensor_zeros, weights[:,:,:,0], tensor_zeros, weights[:,:,:,1], weights[:,:,:,2], weights[:,:,:,3], tensor_zeros, weights[:,:,:,4], tensor_zeros), 2)
            conv_weight = conv_weight.contiguous().view(C_out, C_in, 3, 3)
            
            out_normal = F.conv2d(input=x, weight=conv_weight, bias=bias, stride=stride, padding=padding)

            if math.fabs(theta - 0.0) < 1e-8:
                return out_normal 
            else:
                #pdb.set_trace()
                [C_out,C_in, kernel_size,kernel_size] = weights.shape
                kernel_diff = weights.sum(2).sum(2)
                kernel_diff = kernel_diff[:, :, None, None]
                out_diff = F.conv2d(input=x, weight=kernel_diff, bias=bias, stride=stride, padding=0, groups=groups)
        ...
        def func(x, weights, bias=None, stride=1, padding=0, dilation=1, groups=1):
            ...
            # [C_out,C_in,H_k,W_k] = self.conv.weight.shape
            tensor_zeros = torch.FloatTensor(C_out, C_in, 1).fill_(0).cuda()
            conv_weight = torch.cat((weights[:,:,:,0], tensor_zeros, weights[:,:,:,1], tensor_zeros, weights[:,:,:,2], tensor_zeros, weights[:,:,:,3], tensor_zeros, weights[:,:,:,4]), 2)
            conv_weight = conv_weight.contiguous().view(C_out, C_in, 3, 3)
...
class Conv2d_Diff(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,op_type='cv'):
        ...
        self.groups = groups
        if op_type in ['hvd','dcd']:
            self.weight = nn.Parameter(torch.Tensor(out_channels, in_channels // groups, 1, 5))
        else:
...
class Adapter_MoElayer(nn.Module):
    ...
    def forward(self, x, loss_coef=1):
        ...
        #return y, loss # TODO: hard core
        #y = torch.transpose(y, 1,0)
        return y

class Adapter_MoElayer2(nn.Module):
    ...
    def __init__(self, dim=768, adapter_dim=8,adapter_type=['cv', 'cd','hvd','dcd', 'ad',],noisy_gating=True, k=1):
        super(Adapter_MoElayer2, self).__init__()
        self.noisy_gating = noisy_gating
        self.num_experts = len(adapter_type)
        self.dim = dim
        self.k = k
