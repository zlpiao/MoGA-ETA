...
class FASDataset(Dataset):
    ...
    def __init__(self, data, transforms=None, train=True):
        self.train = train
        self.photo_path = data[0] + data[1]
        self.photo_caption = data[2] + data[3]  # 修改
        self.photo_label = [0 for i in range(len(data[0]))
                            ] + [1 for i in range(len(data[1]))]

        print("len(data[0])", len(data[0]))
        print("len(data[1])", len(data[1]))
        print("len(data[2])", len(data[2]))
    ...
    def __getitem__(self, item):
        if self.train:
            # print("len(self.photo_path)", len(self.photo_path))
            # print("len(self.photo_caption)", len(self.photo_caption))
            # print("len(self.photo_label)", len(self.photo_label))
            # print("---------item",item)
            img_path = self.photo_path[item]
            caption = self.photo_caption[item]
            label = self.photo_label[item]
            img = cv2.imread(img_path)
            img = img.astype(np.float32)
            img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
            ...
            img = Image.fromarray(img.astype(np.uint8)).resize((224, 224))
            img = self.transforms(img)
            return img, label, videoID, img_path, caption
...
def get_dataset(src1_data, src1_train_num_frames, src2_data,
    ...
    data_loaders_list = [src1_train_dataloader_fake, src1_train_dataloader_real, src2_train_dataloader_fake,
                         src2_train_dataloader_real, src3_train_dataloader_fake, src3_train_dataloader_real,
                         src4_train_dataloader_fake, src4_train_dataloader_real, src5_train_dataloader_fake, src5_train_dataloader_real,
                         tgt_dataloader
                         ]
