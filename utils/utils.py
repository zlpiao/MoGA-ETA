...
def sample_frames(flag, num_frames, dataset_name):
  ...
  if dataset_name in ['casia', 'replay', 'oulu', 'msu', 'celeb']:
    ...
    root = '/data/25/zlpiao/zlpiao/data/liuajian_mico_data/ICMO/images_demo/'  
    #       /data/25/zlpiao/zlpiao/data/liuajian_mico_data/
  ...
  if (flag == 0):  # select the fake images
    data = [[
        ...
        i.strip()
        for i in open(dataroot + dataset_name + '_fake_train_caption.txt').readlines()  #修改
    ], []]
    ...
    data = [[],
            ...
            [
                i.strip() for i in open(dataroot + dataset_name +
                                               '_real_train_caption.txt').readlines()
            ]]
    ...
    data = [data[0] + data[0] + data[0] + data[0], [], [], []]  #5个shot，复制了4遍，变成了20个shot
    print('train fake:', dataset_name, len(data[0]), len(data[1]))
    ...
    [
        i.strip()
        for i in open(dataroot + dataset_name + '_real_train_caption.txt').readlines()
    ]]
    #data = [list(set(data[0])), list(set(data[1])),list(set(data[2])), list(set(data[3]))]
